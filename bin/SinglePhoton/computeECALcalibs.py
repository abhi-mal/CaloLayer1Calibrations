...
text_file = open("ecalcalibs_MC_"+suffix+"_filerange"+str(start_index)+"to"+str(end_index_plus_1)+".txt","w")
...
for i in range(0,14):
  ...
  for j in range(0,72):
    ...
    histos_isolation_phi_5x5[i].append( ROOT.TH1F(hname_isolation_phi_5x5,"",4000,0,200) )
    ...
    for event in ntuples:
      ...
      if ETbin>=0 and gen>0 and gen<125 and eta>=0 and eta<56 and phi>=0 and phi<72:
         ...
         if TPGVeto3x3>TPGVeto_cut:
            isolation = l1_summed33_e - l1_center_e
         ...
         if TPGVeto5x5>TPGVeto_cut:
            ...
            isolation = l1_summed55_e - l1_center_e
