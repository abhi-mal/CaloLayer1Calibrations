...
######## LABEL & SAVE WHERE #########
saveWhere='/nfs_scratch/jjbuchanan/calibrations_Jan2017/MonteCarlo/CMSSW_8_0_26_patch1/src/L1Trigger/LauraTriggerTools/bin/SinglePion_batchsplit/'
text_file = open("hcalcalibs_MC_"+suffix+"_all.txt","w")
...
for ETbin in range(0,13):
    ...
    for foldedEta in range(0,28):
        ...
        tf1.Draw("SAME")
        saveas='%spiMinus_SF_hist_ETbin%s_ieta%s_%s.png' % (saveWhere,ETbin,foldedEta+1,suffix)
...
for ETbin in range(0,13):
  ...
  histos_scaleFactor_ETbin_foldedEta_5x5[ETbin].GetXaxis().SetRangeUser(0,28) # Eta range
...
for i in range(0,13):
  ...
  histos[i].GetYaxis().SetRangeUser(0.0,4.0)
  if suffix[-len('_isolation'):] == '_isolation':
...
legend1 = ROOT.TLegend(0.72, 0.6, 0.99, 0.99, "HCAL TPG Et", "brNDC")
if suffix[-len('_isolation'):] == '_isolation':
...
legend1.Draw("same")
saveas='%spiminus_SF_%s_%s.png' % (saveWhere,squaresize,suffix)
canvas.SaveAs(saveas)
