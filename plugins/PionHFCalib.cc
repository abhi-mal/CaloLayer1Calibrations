...
class PionHFCalib : public edm::EDAnalyzer {
	public:
		explicit PionHFCalib(const edm::ParameterSet&);
...
unsigned int const PionHFCalib::N_TOWER_PHI = 72;
unsigned int const PionHFCalib::N_TOWER_ETA = 82;

PionHFCalib::PionHFCalib(const edm::ParameterSet& config) :
	edm::EDAnalyzer(),
	...
	hCorrTowerETCode(N_TOWER_PHI, vector<unsigned int>(N_TOWER_ETA))
{
	...
	genSrc_    = mayConsume<std::vector<reco::GenParticle> >(config.getParameter<edm::InputTag>("genSrc"));
	...
	pions_ = fs->make<TTree>("pions", "Jet quantities");
...
PionHFCalib::~PionHFCalib() {}

	void
PionHFCalib::analyze(const edm::Event& event, const edm::EventSetup& setup)
{
	event_ = event.id().event();

	Handle<HcalTrigPrimDigiCollection> digis;
	if (!event.getByLabel(digis_, digis)) {
		LogError("PionHFCalib") <<
	...
	for(const auto& pion: *objects){
		if (std::abs(pion.eta())<2.976) continue;
		pion_et_=pion.et();
		pion_eta_=pion.eta();
		pion_phi_=pion.phi();
		pion_ieta_=convertHFGenEta(pion.eta());
	...
	for (const auto& digi: *digis) {
		if (digi.id().version() == 1){ //1x1 upgrade
			...
			tp_et_*=hcal[ptbin*9+tmpieta]; //compressed et!!! easily save the et in a vector of ints (divide by 2 later) 
	...
	for(const auto& pion: *objects){
		...
		if (std::abs(pion.eta())<2.967 || pion.pt()<5) continue;//ignore pions in ieta 29
		gen_pt_=pion.pt();
		gen_et_=pion.et();
		gen_eta_=pion.eta();
		gen_phi_=pion.phi();
		...
		//now sum in squares around pion eta 
		//
...
void
PionHFCalib::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
